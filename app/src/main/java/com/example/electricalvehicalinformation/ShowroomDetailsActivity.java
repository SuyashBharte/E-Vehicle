...
public class ShowroomDetailsActivity extends AppCompatActivity {
    ...
    List<ShowroomDetaillist> Personsname1 = new ArrayList<ShowroomDetaillist>();
    ShowroomAdapter profileAdapterr;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (InternetConnection.isInternetAvailable(ShowroomDetailsActivity.this)) {
    ...
    private void loadJSONReviewCategory() {
        ...
        Call<Showroomlist> call=api.getShowrroom();

        call.enqueue(new Callback<Showroomlist>() {
            @Override
            public void onResponse(Call<Showroomlist> call, Response<Showroomlist> response) {
                if (response.isSuccessful()) {
                    System.out.println("Server Response review-- "+response.body().getCategorylist().size());
                    List<ShowroomDetaillist> categoryItems = response.body().getCategorylist();
                    ...
                    if (categoryItems != null && categoryItems.size() > 0) {
                        ...
                        profileAdapterr = new ShowroomAdapter(getApplicationContext(), Personsname1);
                        RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(ShowroomDetailsActivity.this);
            ...
            @Override
            public void onFailure(Call<Showroomlist> call, Throwable t) {
                Toast.makeText(ShowroomDetailsActivity.this, "Error - "+t.getLocalizedMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
